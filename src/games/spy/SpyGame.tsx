  ...
  return (
    <div className="spy-app">
      <header>
        {s.screen !== "join" && s.roomId && (
          <div className="room-badge">
            ...
            {s.copyHint && (
              <span style={{ color: "var(--accent)", marginLeft: 8, fontSize: "0.8rem" }}>
                {s.copyHint}
              </span>
            )}
          </div>
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 6, width: "100%", marginBottom: 6 }}>
                        <button
                          ...
                          className="btn sm"
                          style={{
                            borderColor: s.dictMode === "default" ? "var(--accent-dim)" : undefined,
                            color: s.dictMode === "default" ? "var(--accent)" : undefined,
                          }}
                        ...
                        <button
                          ...
                          className="btn sm"
                          style={{
                            borderColor: s.dictMode === "custom" ? "var(--accent-dim)" : undefined,
                            color: s.dictMode === "custom" ? "var(--accent)" : undefined,
                          }}
          ...
          {s.iAmSpy ? (
            ...
            <>
              <div className="location-box mb1">
                ...
                <div className="loc-name">{s.myRole?.location || "—"}</div>
              </div>
              <div className="notice">
        ...
        <Screen id="screen-game" active={s.screen === "game"}>
          <>
              <div className="timer-wrap">
                ...
                <div className={`timer-display${s.timerUrgent ? " urgent" : ""}`}>
                  {s.timerDisplay}
                </div>
              </div>
              ...
              <div className="game-players mb1">
                {s.players.map(([pid, p]) => (
                  <div key={pid} className={`gp-chip${pid === s.myId ? " me" : ""}`}>
              ...
              <div className="flex mb1">
                {!votePhase && (
                  ...
                  >
                    {s.myVoteRequested ? "✓ Ты за голосование" : "⚖️ Начать голосование"}
                  </button>
                )}
                {s.iAmSpy && (
                  <button
      ...
      <div
        ...
        onClick={(e) => {
          if (e.target === e.currentTarget && !votePhase) s.setModalVoteOpen(false);
        }}
      >
        <div className="modal-box">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
            <h3 style={{ marginBottom: 0 }}>Кто шпион?</h3>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "2.2rem", color: "#e74c3c" }}>
              {s.votePhaseTimer}
            </div>
          </div>
          <p style={{ color: "var(--muted)", fontSize: "0.9rem", marginBottom: "1rem", fontStyle: "italic" }}>
      ...
      >
        <div className="modal-box">
          ...
          <button type="button" className="btn" style={{ fontSize: "1rem" }} onClick={() => s.setModalGuessOpen(false)}>
            Отмена
          </button>
        </div>
      </div>
